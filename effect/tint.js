...
this.fragShader = new PZ.asset.shader(`precision mediump float; uniform sampler2D tDiffuse; varying vec2 vUv; uniform vec3 rgb; uniform float strength; void main(){ vec4 texel = texture2D(tDiffuse, vUv); vec3 tintedColor = mix(texel.rgb, texel.rgb * rgb, strength); gl_FragColor = vec4(tintedColor, texel.a); }`);
